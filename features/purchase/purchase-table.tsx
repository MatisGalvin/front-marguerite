...
import { ClientIdentityCell } from "../../app/(pages)/(back-office)/(main)/clients/components/client-cells/client-cells";
import { BadgeStatus } from "../../app/(pages)/(back-office)/(main)/components/badge-status/badge-status";
import { DetailPurchaseSheet } from "../../app/(pages)/(back-office)/(main)/purchases/components/detail-purchase-sheet/detail-purchase-sheet";
import { PurchaseStatusFilter } from "../../app/(pages)/(back-office)/(main)/purchases/components/filters/filters";
import { UpdatePurchaseSheet } from "../../app/(pages)/(back-office)/(main)/purchases/components/update-purchase-sheet/update-purchase-sheet";

export function PurchaseTable(p: {
  purchases: Purchase[];
  pageSize?: number;
  className?: string;
}) {
      ...
      toast({
        ...
        description: (
          <div>
            ...
            <div className=" mt-2">
              Les commandes ont été marquées commme vues
            </div>
          </div>
        ),
      });
  ...
  const tableActions: TableAction<FormatTableItem>[] = useMemo(
    ...
    [updatePurchase, setIsUpdateSheetOpen],
  );
