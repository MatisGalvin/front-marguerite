...
test.describe('Clients Page Tests', () => {
    ...
    test('should display clients page with data table', async ({ page }) => {
        ...
        await expect(page.getByText('jean.dupont@example.com')).toBeVisible({ timeout: 10000 });
        await expect(page.getByText('0123456789')).toBeVisible({ timeout: 10000 });

        await expect(page.getByText('marie.martin@example.com')).toBeVisible({ timeout: 10000 });
        await expect(page.getByText('0987654321')).toBeVisible({ timeout: 10000 });
    });


    test('should show create client sheet when create button is clicked', async ({ page }) => {
    ...
    test('should handle client creation', async ({ page }) => {
        ...
        if (await createButton.isVisible()) {
            ...
            await expect(page.getByText(/succès|créé|ajouté/i)).toBeVisible({ timeout: 5000 });
        }
    });

    test('should show edit client sheet when edit button is clicked', async ({ page }) => {
        // Mock clients API
        await page.route('**/clients', async (route) => {
            await route.fulfill({
                status: 200,
    ...
    test('should handle client update', async ({ page }) => {
        ...
        if (await editButton.isVisible()) {
            ...
            if (await submitButton.isVisible()) {
                ...
                // Check for success message
                await expect(page.getByText(/succès|modifié|mis à jour/i)).toBeVisible({ timeout: 5000 });
            }
        }
    });
    ...
    test('should handle client deletion', async ({ page }) => {
        ...
        if (await deleteButton.isVisible()) {
            ...
            // Confirm deletion
            const confirmButton = page.getByRole('button', { name: "Continuer" }).last();
            await confirmButton.click();

            // Check for success message
            await expect(page.getByText("Suppression du client")).toBeVisible({ timeout: 5000 });
            await expect(page.getByText("supprimé avec succès")).toBeVisible({ timeout: 5000 });
        }
    });
    ...
    test('should handle deletion error', async ({ page }) => {
        ...
        if (await deleteButton.isVisible()) {
            ...
            await expect(page.getByText(/Echec.*Suppression client/i)).toBeVisible({ timeout: 5000 });
            await expect(page.getByText("Une erreur est survenue")).toBeVisible({ timeout: 5000 });
        }
    });
